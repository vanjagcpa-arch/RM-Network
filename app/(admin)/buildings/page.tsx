...
function HealthDot({ overdue, upcoming, total }: { overdue: number; upcoming: number; total: number }) {
  ...
  if (upcoming > 0) return <span className="inline-flex h-2.5 w-2.5 rounded-full bg-[#16A34A] flex-shrink-0" />;
    ...
    <div className="p-8">
      ...
      {!loading && buildingsList.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            { label: "Buildings", value: buildingsList.length, icon: Building2, color: "text-[#16A34A]", bg: "bg-[#ECFDE8]" },
            { label: "Total Units", value: totalUnits, icon: Home, color: "text-slate-600", bg: "bg-slate-50" },
            { label: "Upcoming Jobs", value: buildingsList.reduce((s, b) => s + b.upcoming, 0), icon: Calendar, color: "text-[#16A34A]", bg: "bg-[#ECFDE8]" },
            ...
            <div key={b.id} className={`bg-white rounded-xl border shadow-sm hover:shadow-md transition-all group ${b.overdue > 0 ? "border-red-200" : "border-slate-200"}`}>
              <Link href={`/buildings/${b.id}`} className="block p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5 flex-1 min-w-0">
                    <div className={`h-9 w-9 rounded-lg flex items-center justify-center flex-shrink-0 ${b.overdue > 0 ? "bg-red-50" : "bg-[#ECFDE8]"}`}>
                      <Building2 className={`h-4 w-4 ${b.overdue > 0 ? "text-red-500" : "text-[#16A34A]"}`} />
                    </div>
              ...
              <div className="flex items-center gap-1 px-5 py-2.5 border-t border-slate-100">
                <button onClick={(e) => { e.preventDefault(); openEdit(b); }}
                  className="flex items-center gap-1 text-xs text-slate-500 hover:text-[#16A34A] transition-colors py-0.5 px-1.5 rounded hover:bg-slate-50">
