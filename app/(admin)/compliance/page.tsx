      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {propertyStats.map(({ prop, cats }) => (
              <tr key={prop.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="px-5 py-3">
                  <Link href={`/buildings/${prop.id}`} className="font-medium text-slate-900 hover:text-[#16A34A] transition-colors">
                    {prop.name}
                  </Link>
        ...
        {allProperties.length === 0 && (
          <div className="py-16 text-center">
            ...
            <Link href="/properties" className="text-sm text-[#16A34A] hover:underline mt-1 inline-block">Add a property →</Link>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col items-center gap-0.5">
      ...
      {(status === "overdue" || status === "due_soon") && (
        <Link href={`/jobs/new?propertyId=${propertyId}&jobCategory=${jobCategory}`}
          className="text-xs text-[#16A34A] hover:underline flex items-center gap-0.5 mt-0.5">
          <Plus className="h-2.5 w-2.5" />Schedule
        </Link>
      )}
    </div>
