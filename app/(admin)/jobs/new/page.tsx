  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {templates.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <select onChange={(e) => loadTemplate(e.target.value)} defaultValue=""
                  className="text-xs rounded-md border border-slate-200 bg-white px-2 py-1 text-slate-600 focus:outline-none focus:ring-2 focus:ring-green-500">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
          ...
          {!form.propertyId ? (
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400">
              <span className="h-3.5 w-3.5 rounded-full border-2 border-green-300 border-t-[#16A34A] animate-spin" />
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {!isBuildingMatch && isTechMatch && <span className="text-[10px] font-semibold bg-[#ECFDE8] text-[#16A34A] px-1.5 py-0.5 rounded-full">Tech available</span>}
                        </div>
                        <p className="text-xs text-slate-500 truncate">{rec.reason} · {rec.jobCount} job{rec.jobCount !== 1 ? "s" : ""} that day</p>
                      </div>
                      {selected && <span className="text-xs text-[#16A34A] font-semibold flex-shrink-0">✓ Selected</span>}
                    </button>
                  );
                })}
              </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
            <User2 className="h-4 w-4 text-[#16A34A]" />
        ...
        {form.tenantEmail && (
          <div className={`rounded-xl border p-5 transition-colors ${sendBookingRequest ? "bg-[#ECFDE8] border-[#CFF8C8]" : "bg-white border-slate-200"}`}>
            ...
            {sendBookingRequest && (
              <p className="mt-3 text-xs text-[#166534] bg-[#ECFDE8] rounded-lg px-3 py-2">
