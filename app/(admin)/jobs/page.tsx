      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search jobs…"
            className="pl-9 pr-4 py-2 text-sm rounded-lg border border-slate-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 w-56" />
        </div>
        <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}
          className="py-2 px-3 text-sm rounded-lg border border-slate-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500">
                ...
                <div key={job.id} className={`flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors group ${isOverdue ? "border-l-2 border-l-red-400" : ""}`}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => openEdit(job)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-xs text-violet-600 hover:underline ml-1 font-medium">
                      Edit
                    </button>
                  </div>
                </div>
