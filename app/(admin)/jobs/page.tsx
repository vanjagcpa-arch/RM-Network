...
import { Plus, Search, ClipboardList, Calendar, Loader2, Download, ExternalLink, HardHat, X } from "lucide-react";
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        {hasFilters && (
          <button
            onClick={() => { setFilterStatus(""); setFilterCategory(""); setFilterTechnician(""); setSearch(""); }}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((job) => {
                ...
                return (
                  <tr key={job.id} className={`hover:bg-slate-50/60 transition-colors group ${isOverdue ? "border-l-2 border-l-red-400" : ""}`}>
                    ...
                    <td className="px-4 py-3.5 hidden md:table-cell">
                      <p className="text-slate-700 max-w-[180px] truncate">{job.property?.name ?? "—"}</p>
                    ...
                    <td className="px-4 py-3.5 hidden lg:table-cell">
                      {job.technician ? (
                        <span
                          className="inline-flex items-center gap-1 text-xs font-semibold rounded-md px-2 py-1 text-white whitespace-nowrap"
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 transition-opacity text-xs text-[#16A34A] hover:underline font-medium"
                      >
                        Edit
                      </button>
                    </td>
